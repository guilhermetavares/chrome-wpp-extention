...
class LeadriveWhatsApp {
    ...
    async handleConversationChange() {
        ...
        if (conversationMatch || this.isInConversation()) {
            ...
            if (phoneNumber && phoneNumber !== this.currentContact) {
                ...
                // Notificar o background script sobre mudança de contato
            ...
            // Se não estamos em uma conversa, limpar contato atual
    ...
    async verifyPhoneNumber(phoneNumber) {
        try {
            console.log('🔍 Verificando número na API:', phoneNumber);
            
            // Enviar mensagem para o background script fazer a chamada da API
            ...
            if (response.success) {
                console.log('✅ Número verificado com sucesso:', response.result);
                
                // Atualizar UI com informações do número
                this.updatePhoneNumberInfo(response.result);
            } else {
                console.error('❌ Erro ao verificar número:', response.error);
            }
    ...
    updatePhoneNumberInfo(phoneInfo) {
        // Criar ou atualizar indicador com informações do número
        let indicator = document.getElementById('leadrive-phone-info');
        
        if (!indicator) {
            indicator = document.createElement('div');
            indicator.id = 'leadrive-phone-info';
        ...
        indicator.innerHTML = `
            ...
            ">
                <div style="font-weight: 600; margin-bottom: 4px;">
                    📞 ${phoneInfo.number || this.currentContact}
                </div>
                <div style="font-size: 11px; opacity: 0.9;">
                    Status: ${phoneInfo.status || 'Verificado'}
                </div>
                ${phoneInfo.name ? `<div style="font-size: 11px; opacity: 0.9;">Nome: ${phoneInfo.name}</div>` : ''}
                ${phoneInfo.company ? `<div style="font-size: 11px; opacity: 0.9;">Empresa: ${phoneInfo.company}</div>` : ''}
            </div>
        `;
